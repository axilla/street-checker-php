...
class APIReportController extends FOSRestController
{
    ...
    public function showAction(Request $request)
    {
        ...
        $reportMyGrade = null;
        if (isset($loggedUser)) {
            $reportGrade = $em->getRepository('AppBundle:ReportGrade')->findOneBy(['owner' => $loggedUser->getId(), 'report' => $report->getId()]);
            if ($reportGrade) {
                $reportMyGrade = $reportGrade->getGrade();
            }
        }
        $avgGradeSum = 0;
        $report->getReportGrades()->map(function (ReportGrade $reportGrade) use (&$avgGradeSum) {
            $avgGradeSum += $reportGrade->getGrade();
        });
        $gradeCount = $report->getReportGrades()->count();
        $avgGrade = $gradeCount ? $avgGradeSum / $gradeCount : 0;
        $serializer = $this->container->get('jms_serializer');
        $report = json_decode($serializer->serialize($report, 'json'));
        $report->myGrade = $reportMyGrade;
        $report->avgGrade = $avgGrade;
     ...
     * @Rest\View
     * @ApiDoc(
     *  description="Return all report categories",
     *  statusCodes = {
     *     200 = "Returned when successful",
     *     204 = "No report categories were found"
     *   },
     *  requirements={
     *
     *  },
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $reportCategories = $em->getRepository('AppBundle:ReportCategory')->findAll();
        if (!$reportCategories) {
            throw new HttpException(204, "No report categories were found");
        }

        $serializer = $this->container->get('jms_serializer');
        $reportCategories = json_decode($serializer->serialize($reportCategories, 'json'));

        return new SCJsonResponse($reportCategories);
    }
...
//        $loggedUser = $auth->getLoggedUserAndSetGrant($request, $reportImage, 'owner');
     ...
     * @Rest\View
     * @ApiDoc(
     *  description="Return Location by criteria",
     *  statusCodes = {
     *     200 = "Returned when successful",
     *     204 = "Returned when no Location matches the criteria"
     *   },
     *  requirements={
     *      {"name"="id", "dataType"="integer", "requirement"="\d+", "description"="Id of Report"}
     *  },
     * parameters={
     *          {"name"="date_before", "dataType"="date", "required"=false, "description"="Search by creation_date before the passed date", "default" = ""},
     *          {"name"="date_after", "dataType"="date", "required"=false, "description"="Search by creation_date after the passed date", "default" = ""},
     *          {"name"="category", "dataType"="integer", "required"=false, "description"="Search by category", "default" = ""},
     *          {"name"="active", "dataType"="integer", "required"=false, "description"="Search by status", "default" = ""},
     *          {"name"="title", "dataType"="string", "required"=false, "description"="Search by title", "default" = ""},
     * }
     * )
     */
    public function searchAction(Request $request)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $fields = $this->container->getParameter('report.listObject');
        $reports = $em->getRepository('AppBundle:Report')->getByCriteria($request->request->all(), $fields);
        if (empty($reports)) {
            return new SCJsonResponse([], 200);
        }
        return new SCJsonResponse($reports);
    }
     ...
     *  statusCodes = {
     *     200 = "Returned when report has successfully been resolved",
     *     400 = "Returned when the report has already been resolved"
     *   },
     *  requirements={
     *      {"name"="id", "dataType"="integer", "requirement"="\d+", "description"="Id of Report"},
     *      {"name"="responseId", "dataType"="integer", "requirement"="\d+", "description"="Id of response that represents the solution"},
     *  }
     * )
     */
    public function resolveAction(Request $request)
    {
        ...
        $responseId = $validator->check($request->get('responseId'), 'integer', 'responseId', true, false);
        $response = $em->getRepository('AppBundle:Report')->find($responseId);

        if (!$report->getResponses()->contains($response)) {
            throw new HttpException(400, 'The response is not in the list of report\'s responses');
        }
        if (!$report->getActive()) {
            throw new HttpException(400, 'The report has already been resolved');
        }

        //check if the number of dislikes is not larger than the percentage defined in parameters.yml
        $reportService = $this->container->get('services.report_validation');
        $reportService->resolveReport($response, true);
     ...
     *     404 = "Returned when the report was not found!",
     *     400 = "Returned when the report has already been graded"
     *   },
     *  requirements={
     *      {"name"="id", "dataType"="integer", "requirement"="\d+", "description"="Id of Report"},
     *      {"name"="grade", "dataType"="integer", "requirement"="\d+", "description"="Grade of the report"}
     *  }
     * )
     */
    public function gradeReportAction(Request $request)
    {
        $auth = $this->container->get('services.auth_service');
        $loggedUser = $auth->getLoggedUser($request);
        if (!$loggedUser) {
            throw new HttpException(403, 'You have to be logged in to grade a report');
        }

        $em = $this->getDoctrine()->getManager();
        $validator = $this->container->get('helpers.input_validator');

        $reportId = $validator->check($request->get('id'), 'integer', 'id', true, false);
        $grade = $validator->check($request->get('grade'), 'integer', 'grade', true, false);
        ...
        $reportGrade = $em->getRepository('AppBundle:ReportGrade')->findBy(
            ['owner' => $loggedUser->getId(), 'report' => $reportId]
        );
        if ($reportGrade) {
            throw new HttpException(400, 'The report has already been graded');
        } else {
            $reportGrade = new ReportGrade();
            $reportGrade->setOwner($loggedUser);
            $reportGrade->setReport($report);
            $reportGrade->setGrade($grade);
            $em->persist($reportGrade);
            $em->flush();

            $serializer = $this->container->get('jms_serializer');
            $context = SerializationContext::create()->setGroups(ReportGrade::getSerializationContextArray());
            $reportGrade = json_decode($serializer->serialize($reportGrade, 'json'));
            return new SCJsonResponse($reportGrade);
        }
     ...
     * @Rest\View
     * @ApiDoc(
     *  description="Returns list of user's resolved Reports",
     *  statusCodes = {
     *     200 = "Returned when successful",
     *     204 = "Returned when no results"
     *   },
     ...
     * )
     */
    public function getOwnerResolvedReportsAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $auth = $this->container->get('services.auth_service');
        $validator = $this->container->get('helpers.input_validator');
        $loggedUser = $auth->getLoggedUser($request, true);
        ...
        $sortingColumn = $validator->check($query->filter('sortingColumn'), 'string', 'sortingColumn', false);
        $sorting = $validator->check($query->filter('sorting'), 'string', 'sorting', false);
        $reports = $em->getRepository("AppBundle:Report")->getList($fields, null, $offset, $limit, $sortingColumn, $sorting,
                                                                   $loggedUser->getId(), false);
        return new SCJsonResponse($reports);
    }
     ...
     * @Rest\View
     * @ApiDoc(
     *  description="Returns list of all of user's reports",
     *  statusCodes = {
     *     200 = "Returned when successful",
     *     204 = "Returned when no results"
     *   },
     ...
     * )
     */
    public function getOwnerReportsAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $auth = $this->container->get('services.auth_service');
        $validator = $this->container->get('helpers.input_validator');
     ...
     * @Rest\View
     * @ApiDoc(
     *  description="Reopen closed report",
     *  statusCodes = {
     *     200 = "Returned when successful",
     *     400 = "Returned when report has already been resolved"
     *   },
     *  requirements={
     *      {"name"="id", "dataType"="integer", "requirement"="\d+", "description"="Id of Report"}
     *  }
     * )
     */
    public function reopenReportAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $auth = $this->container->get('services.auth_service');
        $validator = $this->container->get('helpers.input_validator');
        ...
        $report = $em->getRepository('AppBundle:Report')->find($reportId);
        if (!$report) {
            throw new HttpException(404, "Report with id = $reportId, has not been found!");
        }
